...
from CLIP.adapter import CLIP_Inplanted_groupPNmixAfterConv_groupMaxNensembleOut
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='./medical_data/data')
    ...
    parser.add_argument('--nGroups', type=int, default=11)
    parser.add_argument('--bAffine', type=bool, default=False)
    parser.add_argument('--prob', type=float, default=0.25)
    parser.add_argument('--nMaxN', type=int, default=5)
    parser.add_argument('--bBySum', type=int, default=1)  # 1:sum,0:var;2:max;
    parser.add_argument('--shot', type=int, default=4)
    parser.add_argument('--batch_size', type=int, default=8)
    parser.add_argument('--obj', type=str, default='Chest')
    ...
    model = CLIP_Inplanted_groupPNmixAfterConv_groupMaxNensembleOut(clip_model=clip_model, features=args.features_list,nGroups=args.nGroups,bAffine=args.bAffine,nMaxN=args.nMaxN,bBySum=args.bBySum).to(device)
    model.eval()
    ...
    if args.bAffine:
        pono_optimizer = torch.optim.Adam(list(model.pono.parameters()), lr=args.learning_rate, betas=(0.5, 0.999))
        ...
        for (image, gt, label) in train_loader:
            ...
            if prob < args.prob:
                with torch.cuda.amp.autocast():
                    _, seg_patch_tokens, det_patch_tokens,rand_index = model(image,True)
                    target_a = image_label
                    target_b = image_label[rand_index]
                    # compute output
                    ...
                    for layer in range(len(det_patch_tokens)):
                        ...
                        det_loss += 0.5*loss_bce(anomaly_score, target_a_var)+0.5*loss_bce(anomaly_score, target_b_var)

                    if CLASS_INDEX[args.obj] > 0:
                        # pixel level
                        seg_loss = 0
                        mask = gt.squeeze(0).to(device)
                        mask_mix = mask[rand_index]
                        mask =mask+mask_mix
                        ...
                        for layer in range(len(seg_patch_tokens)):
                            ...
                            H = int(np.sqrt(L))
                            anomaly_map = torch.nn.functional.interpolate(anomaly_map.permute(0, 2, 1).view(B, 2, H, H),
                                                                          size=args.img_size,
                                                                          mode='nearest')
                ...
                with torch.cuda.amp.autocast():
                    _, seg_patch_tokens, det_patch_tokens = model(image,False)
                    seg_patch_tokens = [p[:, 1:, :] for p in seg_patch_tokens]
                    det_patch_tokens = [p[:, 1:, :] for p in det_patch_tokens]
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            if CLASS_INDEX[args.obj] > 0:
                ...
                for idx, p in enumerate(seg_patch_tokens):
                    ...
                    anomaly_map_few_shot = F.interpolate(torch.tensor(anomaly_map_few_shot),size=args.img_size, mode='bilinear', align_corners=True)
